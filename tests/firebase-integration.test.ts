...
describe('payload estruturado e determinístico', () => {
  it('publica SOC em schedule_periods/schedule_assignments compatíveis', () => { const payload = buildStructuredPublicationPayload(regularState(), team('soc'), user()); expect(payload.kind).toBe('REGULAR'); expect(payload.period.scheduleType).toBe('SOC'); expect(payload.assignments).toHaveLength(2); expect(payload.assignments[0]).toMatchObject({ teamId: 'soc', assignmentType: 'WORK_SHIFT', schemaVersion: 2 }); });
